...
Bots fight by performing [bot_moves](#bot_moves), looping their [moves_sequence](../elements/bot.md#moves_sequence) at a fixed interval (ticks). Bots perform moves at the same time.
# bot_moves
## Overview
Moves are the actions the [bots](../elements/bot.md) perform during the match.
Each bot has a certain amount of [stamina](../elements/bot.md#stamina) that each move can consume or recover. When the bot's stamina reaches 0 an [Empty move](#Empty) gets inserted between the current and the next move in the bot's moves sequence for that loop.
## Attack
...
The assignment of traits is controlled by the [bots_simulation_system](../systems/bots_simulation_system.md).
# match_time
...
- One or both bots reach 0 [health](../elements/bot.md#health)
